...
**Live-run finding #3 (2026-09-20) — reopening again, the interval itself is off by a large margin.** Resumed extraction a third time with 1.1s pacing in place; it failed **again, at the exact same point** (400 messages). The fact that pacing barely changed anything is itself informative: back-of-envelope math (assuming Gmail's documented ~5 quota units per `messages.get`, 50/batch = ~250 units/burst, failing after 8 completed batches) puts the account's actual per-minute ceiling around **~2,000 units/minute** — roughly 7x below the spec's original assumption of "250 units/second sustained" (which would imply ~15,000/minute). 1.1s of spacing between 250-unit bursts works out to ~227 units/sec sustained, still far over whatever the real cap is. **This isn't something to keep guessing at by trial and error against the live account** — Mark can check the actual configured limit directly in Google Cloud Console under APIs & Services → Gmail API → Quotas, which would replace this estimate with the real number and let the interval (or `HTTP_BATCH_SIZE`) be set correctly the first time rather than through more repeated live-account attempts. No data lost — still 400 messages stored, no checkpoint written. Not resuming again until either the real quota number is confirmed or a substantially more conservative interval is chosen.
