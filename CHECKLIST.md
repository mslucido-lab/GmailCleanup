...
**Live-run finding #3 (2026-09-20) — confirmed against the real quota, fix needed.** Resumed extraction a third time with 1.1s pacing in place; it failed **again, at the exact same point** (400 messages), which motivated checking the actual configured limit instead of continuing to guess. Mark pulled it from Google Cloud Console (APIs & Services → Gmail API → Quotas): **"Units per minute per user" = 6,000** — this project's active limit has been reduced from the "previous quota" of 15,000 shown in the same console, which is what the original spec's "250 units/second sustained" assumption (≈15,000/minute) was implicitly based on.

**Fix needed:** at ~5 quota units per `messages.get` (Gmail's documented cost) and 50 messages/batch (~250 units/batch), the exact ceiling is 6,000 ÷ 250 = 24 batches/minute (2.5s/batch). Recommend targeting **~19–20 batches/minute (~3.0–3.2s between batches)** to leave headroom for `messages.list` calls and retries drawing from the same per-minute bucket. Update `GMAIL_METADATA_BATCH_INTERVAL_SECONDS` from `1.1` to somewhere in that range. **Known consequence:** at ~20 batches/minute, the full 412,847-message main pass will take roughly **7 hours**, not the couple of hours originally assumed — flagging this now so it's an informed expectation, not a surprise mid-run. (The quota row is marked "Adjustable: Yes" in the console if Mark ever wants to request an increase instead, but for a one-time personal pull, accepting the slower correctly-paced run seems the simpler choice.) No data lost — still 400 messages stored, no checkpoint written. Not resuming again until the interval is updated.
